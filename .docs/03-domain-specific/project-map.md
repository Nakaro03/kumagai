...
新しい予測ターゲット・アーキテクチャを提案する前に、このファイルで既存の到達点を確認すること。詳細な数値・実験設計は各リンク先を参照——`docs/*.md`は2026-08-22時点で凍結された過去の記録（[設計記録の方針](../02-workflow/documentation-policy.md)参照）、それ以降の更新は対応するGitHub Issueの固定コメントで行う。
