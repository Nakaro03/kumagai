#!/usr/bin/env python3
"""
UnifiedVGAETD（Φ(z, year)）学習済み checkpoint からインタラクティブ HTML を出力する。

チェックポイントは ``run_train_unified_vgae_td`` が保存した辞書（state_dict + year_min/year_max）を想定。
生の state_dict のみの場合は --year-min / --year-max を必須にする。
...
def main():
    ...
    default_out = repo / "pnode_patent_runner/outputs/cope_landscape/map_cope_alt_dark_td.html"
    default_html_template = repo / "pnode_patent_runner/interactive_vector_field_alt_dark.html"

    parser = argparse.ArgumentParser(
        description=f"{METHOD_SHORT_NAME_TD} — Φ(z, year) 潜在マップ + ベクトル場 HTML",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="checkpoint に year 情報が無いとき必須（学習年範囲の下限）",
    )
    parser.add_argument(
        "--year-max",
        type=int,
    ...
    if isinstance(raw_ckpt, dict) and "state_dict" in raw_ckpt:
        ...
        if args.year_min is None or args.year_max is None:
            raise SystemExit(
                ...
                " --year-min と --year-max を指定するか、run_train_unified_vgae_td で保存した辞書を使ってください。"
            )
    ...
    page_title = f"{METHOD_SHORT_NAME_TD}: Φ(z,y) map + field"
    heading = f"{METHOD_SHORT_NAME_TD} — Φ(z, 年) ＋ −∇Φ（時間依存ポテンシャル）"
    ...
    print(f"  Φ(z,y) の y は各スライダー年に一致（meta: {meta_ym}..{meta_yM}）")
