...
def future_link_auc_scores(
    ...
    """`year_prev` の潜在（必要なら過去数年の履歴）から 1 年先を予測し、`year_next` の二部リンクで AUC 用ラベル・スコアを返す。"""
    model.eval()
    ...
    with torch.no_grad():
        ...
        z_hist.append(z)
        if getattr(model, "pnode_energy_td", False):
            z_pred = model.predict_future(z_hist, int(year_prev))
        elif getattr(model, "time_dependent_potential", False) and getattr(model, "variant", None) == "pnode":
            z_pred = model.predict_future(z_hist, int(year_prev))
        else:
...
def evaluate_val_future_link_metrics(
    ...
    """
    最終2年 (y_{T-1}→y_T) の future-link 予測について ROC-AUC と Average Precision を返す。
...
def evaluate_val_future_link_metrics_for_years(
    ...
    """
    指定した ``(year_prev, year_next)`` の future-link について ROC-AUC / AP を返す。
